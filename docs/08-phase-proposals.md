...
> items under C say what's done; the rest of C stands. The **global coverage map** from
> **E** landed the same day, on the registry rather than E's slim index — see E.
...
> **Partly landed (2026-08-17), outside a phase.** The global coverage map is done, built
> on the **registry** instead of waiting for the slim index: `index.json` entries now carry
> `center` and `facility_count`, validated against the region file so they can't drift, and
> the landing view pins every published region without downloading one. That was enough for
> the map; it is *not* enough for cross-region search, which still needs the slim index
> below (a name/category/agency index is a different shape from a per-region summary).
...
- [x] **Global coverage map** — the landing view when no region is selected: region
      markers with facility counts, clustered where they overlap, click to enter a region.
      *Landed on the registry's `center`/`facility_count`.* Still open: pinning
      `requested` / `in_progress` regions, which needs a center for a region nobody has
      located yet — the queue stays in the About panel until then.
