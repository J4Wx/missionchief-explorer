...
function FacetGroup({ label, facetKey, note, options, selected, onToggle }: GroupProps) {
  ...
  return (
    <div className="border-t border-slate-100">
      ...
      {open && note && <p className="px-4 pb-1 text-xs text-slate-400">{note}</p>}
      {open && (
            ...
            return (
              <li key={option.value}>
                <button
                  ...
                  className="flex w-full items-center gap-2 rounded px-2 py-1 text-left text-sm hover:bg-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-500"
                >
                  <span
                    aria-hidden="true"
                  ...
                  <span className="min-w-0 flex-1 truncate text-slate-700" title={option.label}>
                    {option.label}
                  </span>
                  <span className="tabular-nums text-xs text-slate-400">{option.count}</span>
                </button>
              </li>
            )
          })}
      ...
      <div className="flex items-center gap-2 px-4 py-2">
        ...
        {active > 0 && (
          <button
            type="button"
            onClick={onClear}
            className="rounded px-1.5 py-0.5 text-xs font-medium text-blue-700 hover:bg-blue-50"
          >
            Clear all
          </button>
        )}
        ...
        <div className="flex flex-wrap gap-1 px-4 pb-2">
          {chips.map(({ key, value }) => (
            <button
              ...
              className="inline-flex items-center gap-1 rounded-full bg-slate-100 py-0.5 pl-2 pr-1 text-xs text-slate-700 hover:bg-slate-200"
            >
